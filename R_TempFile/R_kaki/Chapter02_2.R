...
axis(1, at=1:5, labels = c("월","화","수","목","금"))
axis(2, ylim=c(0,400))
...
# density
barplot(apple, main="Fruits", xlab="Day", ylab="Qty", border="blue",
...
legend(10, 450, c("월","화","수","목","금"), cex=0.8, fill=rainbow(5))
...
barplot(fruits11, main="Fruits", ylab="Qty", ylim = c(0,1100), col=rainbow(3), space=0.1,
        cex=0.8, cex.axis=0.8, las=1, names.arg=c("월","화","수","목","금"))
...
data <- read.csv("data/학생시험결과_전체점수.csv")
data

treemap(data, vSize="점수", index="점수")
treemap(data, vSize="조", index="조")

treemap(data, vSize="조", index=c("점수","이름"))
...
data <- read.csv("data/학생별전체성적.csv")
data
nrow(data)

# 1번 이름 컬럼을 rowname으로 설정하고 삭제함
row.names(data) <- data$이름
rownames(data)
data <- data[2:7]
...
stars(data, flip.labels = F, draw.segments = F, frame.plot = T, full = T,
      main = "학생별 과목별 분석 다이어그램")

label <- names(data)
value <- table(label)
pie(value, labels = label, radius = 0.1, cex = 0.6, col = NA)
...
# ggplot(data, aes(x=x축데이터, y=y축데이어)) + geom 함수

install.packages("ggplot2")
library(ggplot2)

data <- read.table("data/학생별국어성적.txt", header=T, sep="")
data
...
data <- read.csv("data/학생별과목별성적_국영수.csv", header=T, sep=",")
data
...
ggplot(transData, aes(x=이름, y=점수, fill=과목)) + geom_bar(stat="identity") + 
...
ggplot(data[,1:2], aes(x=국어, y=reorder(이름,국어))) + geom_point(size=5) +
  theme_bw() +                                      
...
ggplot(data[,1:2], aes(x=국어, y=reorder(이름,국어))) + 
...
data <- read.csv("data/학생별과목별성적_3명.csv")
